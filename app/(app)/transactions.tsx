...
export default function TransactionsScreen() {
  ...
  useEffect(() => {
    if (user?.uid) fetchTransactions(user.uid).catch(() => {});
  }, [user?.uid]);
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter((t) => {
        ...
        return cat.includes(q) || (t.note && t.note.toLowerCase().includes(q));
      });
    }
    return result;
  ...
  const handleEdit = async () => {
    ...
    setSaving(true);
    try {
      await updateTransaction(editTx.id, { amount: numAmount, note: editNote });
      setEditTx(null);
    ...
    <View className="flex-1" style={{ backgroundColor: c(Colors.background) }}>
      ...
      <View className="px-4 -mt-3">
        <View className="flex-row gap-3 mb-4">
          <View className="flex-1 rounded-2xl p-3" style={{ backgroundColor: c(Colors.surface), shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 }}>
        ...
        <View className="flex-row items-center rounded-2xl px-4 mb-3" style={{ height: 48, backgroundColor: c(Colors.surface) }}>
          <Ionicons name="search-outline" size={20} color={c(Colors.textSecondary)} />
          <TextInput
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-4">
          <TouchableOpacity
            ...
            style={{ backgroundColor: catFilter === "all" ? Colors.primary : c(Colors.surface) }}
          >
            <Text className="text-sm font-inter-medium" style={{ color: catFilter === "all" ? c(Colors.surface) : c(Colors.textPrimary) }}>All</Text>
          </TouchableOpacity>
          {[...INCOME_CATEGORIES, ...EXPENSE_CATEGORIES].map((cat) => (
            <TouchableOpacity
              ...
              style={{ backgroundColor: catFilter === cat.id ? cat.color : c(Colors.surface) }}
            >
              <Ionicons name={cat.icon as any} size={14} color={catFilter === cat.id ? c(Colors.surface) : cat.color} style={{ marginRight: 6 }} />
              <Text className="text-sm font-inter-medium" style={{ color: catFilter === cat.id ? c(Colors.surface) : c(Colors.textPrimary) }}>{cat.label}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
        ...
        <ScrollView showsVerticalScrollIndicator={false} className="mb-32">
          {filtered.length === 0 && !isLoading && (
            <View className="items-center py-16">
              ...
              <Text className="text-base font-inter-semibold mb-1" style={{ color: c(Colors.textPrimary) }}>No Transactions Found</Text>
          ...
          {filtered.map((tx) => {
            ...
            return (
              <View key={tx.id} className="rounded-2xl p-4 mb-2" style={{ backgroundColor: c(Colors.surface), shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.03, shadowRadius: 4, elevation: 1, borderWidth: 1, borderColor: c(Colors.border) }}>
                <View className="flex-row items-center justify-between">
                  <View className="flex-row items-center gap-3 flex-1">
                    ...
                    <View className="flex-1">
                      <Text className="text-base font-inter-semibold" style={{ color: c(Colors.textPrimary) }}>{cat.label}</Text>
                      <Text className="text-xs font-inter-regular mt-0.5" style={{ color: c(Colors.textSecondary) }}>
                  ...
                  <View className="items-end gap-1">
                    ...
                    <View className="flex-row gap-2">
                      ...
                      >
                        <Ionicons name="create-outline" size={16} color={c(Colors.textSecondary)} />
                      </TouchableOpacity>
                      <TouchableOpacity onPress={() => handleOpenDelete(tx.id)}>
      ...
      <Modal visible={!!editTx} transparent animationType="fade">
        <View className="flex-1 bg-black/40 items-center justify-center px-8">
          <View className="rounded-3xl p-6 w-full" style={{ backgroundColor: c(Colors.surface) }}>
            <Text className="text-lg font-inter-bold text-center mb-4" style={{ color: c(Colors.textPrimary) }}>Edit Transaction</Text>
            {editTx && (
              <>
                <Text className="text-xs font-inter-medium mb-1" style={{ color: c(Colors.textSecondary) }}>{getCategory(editTx.category).label} · {editTx.type === "income" ? "Income" : "Expense"}</Text>
                <TextInput
                ...
                <View className="flex-row gap-3">
                  ...
                  <View className="flex-1"><Button title="Save" onPress={handleEdit} loading={saving} /></View>
                </View>
              </>
            )}
          </View>
